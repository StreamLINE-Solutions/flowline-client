...
class _ServerPageState extends State<ServerPage> {
  ...
  void initState() {
    ...
    // FlowLINE support (incoming-only) : démarre le service automatiquement au
    // lancement — les permissions (capture écran…) sont demandées au 1er run,
    // le service démarre dès que la capture est accordée.
    if (bind.isIncomingOnly() && !gFFI.serverModel.isStart) {
            ...
            ElevatedButton.icon(
                ...
                onPressed: () {
                  if (bind.isIncomingOnly()) {
                    // FlowLINE support (incoming-only) : démarrage direct, pas
                    // d'avertissement anti-scam (le service est le cœur du mode).
